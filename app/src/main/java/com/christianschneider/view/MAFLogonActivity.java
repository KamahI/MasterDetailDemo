 ...
 * it initializes the NorthwindApp and creates/starts the Intent for the SalesOrder List UI
 *
 */
public class MAFLogonActivity extends AppCompatActivity implements LogonListener
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        mLogonUIFacade.setDefaultValue(LogonCore.SharedPreferenceKeys.PREFERENCE_ID_PASSWORD, getString(R.string.PASSWORD));
        mLogonUIFacade.setDefaultValue(LogonCore.SharedPreferenceKeys.PREFERENCE_ID_SECCONFIG, getString(R.string.SECCONFIG));
        mLogonUIFacade.setDefaultValue(LogonCore.SharedPreferenceKeys.PREFERENCE_ID_MOBILEUSER, getString(R.string.MOBILEUSER));
        ...
        mLogonUIFacade.isFieldHidden(LogonCore.SharedPreferenceKeys.PREFERENCE_ID_SUPSERVERPORT, false);
        mLogonUIFacade.isFieldHidden(LogonCore.SharedPreferenceKeys.PREFERENCE_ID_SECCONFIG, false);
        ...
        mLogonUIFacade.isFieldHidden(LogonCore.SharedPreferenceKeys.PREFERENCE_ID_MOBILEUSER, false);
        ...
        mLogonUIFacade.showSplashScreen(true);
    }
    ...
    public void onLogonFinished(String errorMessage, boolean isUserLoggedOn, LogonContext logonContext)
    {
        ...
        if (isUserLoggedOn && errorMessage.isEmpty())
        {
            ...
            try {
                Log.i(TAG, "Logon finished. logoncontext: enpoint URL: " + logonContext.getEndPointUrl() + "\n resource path: " + logonContext.getResourcePath().toString() + "\n is logged on: " + logonContext.isUserLoggedOn() +
                        "\n ServerHostName: " + logonContext.getServerHostName() + "\n urlSuffix: " + logonContext.getUrlSuffix() + "\n is registered: " + logonContext.isRegistered());
            } catch (LogonManager.LogonManagerException e) {
                e.printStackTrace();
            }
            try {
                SalesOrderDataSingleton.initialize(this);
            } catch (LogonCoreException e) {
                e.printStackTrace();
            }

            Intent goToNextActivity = new Intent(this, SalesOrderListActivity.class);
